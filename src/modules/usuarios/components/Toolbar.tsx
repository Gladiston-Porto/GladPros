  ...
  const handleExport = async (format: 'csv' | 'pdf') => {
    ...
    try {
      if (scopeNow === 'selected') {
        if (users.length === 0) {
          showToast({ title: 'Aviso', message: 'Nenhum usuário selecionado para exportar', type: 'info' });
          return;
        }
        if (format === 'csv') {
        ...
        if (!onExportAllFiltered) {
          showToast({ title: 'Erro', message: 'Exportação de todos os filtrados não suportada', type: 'error' });
          return;
        }
